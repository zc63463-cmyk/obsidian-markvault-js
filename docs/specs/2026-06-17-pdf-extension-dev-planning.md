...
> 日期: 2026-06-17 | 版本: v1.2 (需求校准) | 状态: CALIBRATED
>
> v1.1 校准变更（2026-06-17）:
> - ① 标注方式：去掉 pdf-ink（画笔），仅保留 pdf-highlight（荧光笔）+ pdf-area（方框区域）
> - ② 预览展示：去掉 previewImage(base64截图) → 改为 previewNote(额外MD字段)
> - ③ 定位和持久化：确认双定位策略不变，PDF 位置永不变不需要漂移恢复
>
> v1.2 校准变更（2026-06-17）:
> - ④ 去掉 previewNote 字段：PDF 标注的 text 本身就充当预览内容（默认留空，用户自行键入）
>   - 原因：PDF.js getTextContent() 在扫描版/公式/图表/复杂排版场景极不可靠
>   - 用户工作流：截图→发豆包等AI→获取MD文本→粘贴到text→MarkdownRenderer.renderMarkdown()渲染
>   - 优势：不新增字段、与MD标注共享同一渲染管线、零额外开发成本
>
> 关联文档:
...
| 修改项 | 文件 | 当前值 | 扩展为 |
|--------|------|--------|--------|
| Annotation.kind | `types/annotation.ts:37` | `'inline' \| 'block' \| 'span' \| 'region'` | + `'pdf-highlight' \| 'pdf-area'` (去掉 pdf-ink) |
...
| ~~Annotation.previewNote~~ | `types/annotation.ts` | **不存在** | v1.2校准: 不再新增。PDF标注的 text 字段本身充当预览内容，默认留空用户键入 |
...
| KIND_COLORS | `graph-data-builder.ts:63-68` | 4 种 | + pdf-highlight / pdf-area (去掉 pdf-ink) |
...
interface Annotation {
  ...
  // ── 注: 不再新增 previewNote 字段 ────────
  // PDF 标注的 text 字段本身就充当"用户键入的预览内容"
  // text 默认留空，用户截图→豆包→MD文本→粘贴到 text → MD渲染
  // 这与 MD 标注的 text 自动提取不同，但渲染路径完全一致

  // ── 其余字段 100% 保留 ──────────────────
  text: string;               // MD标注: 自动提取; PDF标注: 默认留空，用户自行键入MD文本
  note: string;
  color: string;
  tags: string[];
...
// ── AnnotationKind 扩展（v1.1 校准: 去掉 pdf-ink）──
type AnnotationKind =
  | 'inline' | 'block' | 'span' | 'region'     // MD 标注（现有）
  | 'pdf-highlight' | 'pdf-area';               // PDF 标注（新增，2种）
```
...
| **文本内容: text** | 搜索/关系/预览 | **默认留空**，用户自行键入（截图→豆包→MD） | 用户手动填入 |
...
| 文本获取 | 直接从 MD 源码截取 | **默认留空，用户键入**（截图→豆包→MD） |
| 区域标注 | 行号范围 | 归一化坐标矩形 |
...
| ~~自由画笔~~ | ~~FragmentSelector + SvgSelector(`<path>`)~~ | 已去掉（v1.1 校准） |
| 跨页区域 | RangeSelector(FragmentSelector₁, FragmentSelector₂) | `page=5..7` |

---

### 3.5 Preview 策略：MD 标注 vs PDF 标注

**核心原则：MD 标注保持自动记录方式不变。PDF 标注的 text 默认留空，由用户自行键入内容（典型：截图→豆包→MD文本→粘贴），不再新增 previewNote 字段。**

#### 为什么 PDF 标注不能自动提取 text？

PDF.js 的 `getTextContent()` 看似能提取文本，但实际场景中**极不可靠**：

| PDF 场景 | getTextContent 结果 | 可靠？ |
|-------------------------------|--------|
| 纯文本 PDF | ✅ 正确提取 | 可用 |
| 扫描版 PDF (图片型) | ❌ 无 OCR，返回空 | **不可靠** |
| 数学公式 (LaTeX 渲染) | ❌ 乱码或空 | **不可靠** |
| 图表/图示/图片 | ❌ 非文本内容，空 | **不可靠** |
| 复杂排版 (多栏/脚注/侧栏) | ⚠️ 顺序错乱 | **不可靠** |
| 特殊字体/编码 | ⚠️ 可能乱码 | **不可靠** |

**结论：PDF 标注自动提取 text 的成功率远低于 MD 标注，与其"有时有有时没有"不如统一留空，让用户主动填入。**

#### 统一策略对比

| 维度 | MD 标注（现有，不变） | PDF 标注（荧光笔+方框，统一策略） |
|------|---------------------|-------------------------------|
| **text 来源** | 自动提取（锚点文本/选中文本） | **默认留空，用户自行键入** |
| **text 渲染** | `MarkdownRenderer.renderMarkdown()` | **完全一致**，同一渲染管线 |
| **用户工作流** | 不需要额外操作 | 截图→发豆包等AI→获取MD文本→粘贴到text |
| **新增字段** | 无 | **无**（不需要 previewNote） |

#### 为什么不需要 previewNote 字段？

之前规划的 `previewNote` 作为 PDF 标注的额外空间，现在简化了：

- text 字段本身就是可编辑的 MD 文本，支持 `MarkdownRenderer.renderMarkdown()` 渲染
- MD 标注的 text 是自动提取的（只读），PDF 标注的 text 是用户键入的（可编辑）——**数据结构完全相同，只是填充方式不同**
- 不需要额外字段，减少复杂度
- 侧边栏/详情页的渲染逻辑零改动——都是读 `annotation.text` → `MarkdownRenderer.renderMarkdown()`

#### 典型使用场景

```
场景1: 教材定理证明
  1. 用户在 PDF 上选中一段定理 → 创建 pdf-highlight 标注
  2. 标注详情中 text 默认为空 → 用户截图该区域
  3. 截图发给豆包 → "请将这个数学定理转为 Markdown 文本"
  4. 豆包返回: "$$\\int_0^1 f(x)dx = F(1) - F(0)$$（牛顿-莱布尼茨公式）"
  5. 用户粘贴到 text → 标注详情页 MD 渲染出漂亮的公式

场景2: 图表区域标注
  1. 用户在 PDF 上框选一个折线图 → 创建 pdf-area 标注
  2. 截图发给豆包 → "请描述这个图表的关键信息"
  3. 豆包返回 MD 文本（含表格/列表）
  4. 粘贴到 text → 侧边栏/图谱中可搜索、可关联

场景3: 扫描版古籍
  1. 用户框选一段古文 → pdf-area
  2. 截图→豆包 OCR+转写 → 获取 MD 文本
  3. 粘贴到 text → 即使扫描版 PDF 也能有完整的可搜索文本
```

#### 可选优化（Phase 2+）

- 自动尝试 `getTextContent()`，如果成功则**预填充 text**，用户仍可编辑覆盖
- 这不是 Phase 1 的需求，只是后续体验优化
...
→ text 默认留空（用户可自行截图→豆包→MD文本→粘贴填入）
...
- 侧边栏显示: 标注列表 + text(默认留空,用户可填入MD文本,MD渲染)
...
**v1.1 校准**: 方框区域比画笔好实现一个量级——只需 4 个浮点数即可定位，覆盖图表、公式块、图片等 80%+ 区域标注需求。画笔（svgPath + Douglas-Peucker）数据量和复杂度远超方框，且实际使用频率低。

### 4.3 ~~自由画笔（PDFInk）~~ — 已去掉（v1.1 校准）

**去掉理由**:
1. 实现难度 ★★★★ → 方框区域 ★★★★★，方框比画笔简单一个量级
2. 存储量: 画笔每笔 ≈ 几百个坐标点 vs 方框 = 4 个浮点数
3. 覆盖度: 方框覆盖 80%+ 区域标注需求（图表、公式块、图片区域），画笔仅补充自由标注
4. 优先级: 荧光笔+方框是 MVP 最低组合，画笔可后续 Phase 3 再考虑
...
- 高亮优先级: pdf-highlight > pdf-area（避免遮挡）
...
function buildPDFTarget(annotation: Annotation): W3CTarget {
  ...
  // 注: pdf-ink (svgPath → SvgSelector path) 已去掉（v1.1 校准）
...
function parsePDFSelectors(selectors: W3CSelector[]): PDFSelector {
  ...
  for (const s of selectors) {
    ...
    if (s.type === 'SvgSelector') {
      ...
      // 注: svgPath 解析已去掉（v1.1 校准，不再支持 pdf-ink）
    }
...
async function createPDFAnnotation(
  ...
  const annotation = buildAnnotation({
    ...
    docType: 'pdf',
    format: 'pdf',
    kind: params.pdfKind,   // 'pdf-highlight' | 'pdf-area'
...
interface PDFAnnotationCreateParams extends AnnotationCreateParams {
  pdfKind: 'pdf-highlight' | 'pdf-area';
  pdfSelector: PDFSelector;
  // text 默认留空（v1.2校准），用户后续自行键入或粘贴MD文本
}
```
...
4. text 默认留空（v1.2校准: PDF文本提取不可靠，用户后续自行键入）
...
5. text 默认留空（用户可后续键入：截图→豆包→MD→粘贴）
...
**核心原则**: MD 标注保持自动记录方式不变，PDF 标注的 text 默认留空+用户键入。两者在侧边栏中完全同构展示（同一渲染管线 `MarkdownRenderer.renderMarkdown()`），只是内容来源不同：

- **MD 标注**: text 自动提取 → `MarkdownRenderer.renderMarkdown()` 渲染（与当前一致，零改动）
- **PDF highlight/area**: text 默认留空 → 用户截图→豆包→MD文本→粘贴 → 同样 MD 渲染

| 标注类型 | 侧边栏预览 | 点击跳转 |
|---------|-----------|---------|
| MD inline/block/span | CM6 编辑器内高亮预览 | 打开 MD 文件定位偏移 |
| MD region | 区域文本摘要 | 打开 MD 文件定位行 |
| **PDF highlight** | **text默认留空,用户可填入MD渲染** | **打开 PDF 跳转页码** |
| **PDF area** | **text默认留空,用户可填入MD渲染** | **打开 PDF 跳转区域** |
...
```
[📝 高亮] [🔲 区域] [⚙ 设置] ← PDF 标注工具栏 (v1.1: 去掉画笔按钮)
```
...
> v1.2 校准: 不再新增 previewNote 字段。PDF 标注的 text 本身就是用户键入的预览内容（默认留空），
> 与 MD 标注共享同一渲染管线 MarkdownRenderer.renderMarkdown()，零额外开发成本。

### Phase 3: 跨文档关系 + 高级特性 (2-3 周)
...
| ~~自由画笔~~ (可选) | ~~3天~~ | ~~P3~~ (v1.1 校准: 降为可选) |
| 跨页区域标注 | 2天 | P2 |
| 测试 | 2天 | P0 |

**总计**: ~7 天（不含画笔）/ ~10 天（含画笔）

> v1.1 校准: 画笔从 P0 降为 P3 可选。荧光笔+方框区域覆盖 80%+ 场景，画笔可后续按需开发。
...
- [ ] Annotation 接口扩展设计评审（docType + pdfSelector，不再新增 previewNote）
...
| PDF 标注预览 | base64截图 vs 文本提取 vs 新增字段 | **text字段本身(默认留空,用户键入MD)** | v1.2校准: 不新增字段，text既是定位辅助也是预览容器 |
...
│      ↓                                                       │
│  PDF overlay 渲染 (SVG rect 高亮/区域覆盖层)                  │
│      ↓                                                       │

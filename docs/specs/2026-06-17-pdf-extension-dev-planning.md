...
> 日期: 2026-06-17 | 版本: v1.1 (需求校准) | 状态: CALIBRATED
>
> v1.1 校准变更（2026-06-17）:
> - ① 标注方式：去掉 pdf-ink（画笔），仅保留 pdf-highlight（荧光笔）+ pdf-area（方框区域）
> - ② 预览展示：previewImage(base64截图) → previewNote(可选MD文本)，默认留白，用户自行填入
...
| Annotation.previewNote | `types/annotation.ts` | **不存在** | 新增 `string` (可选MD文本, 默认空) 替代 previewImage |
...
interface Annotation {
  ...
  // ── 新增: 自定义预览笔记（替代 previewImage） ──
  previewNote?: string;       // 可选 MD 文本，默认留白，用户可自行填入
                               // 典型用法: 截图→发会话AI→获取MD文本→粘贴此处
                               // 标注详情页支持 MD 渲染

  // ── 其余字段 100% 保留 ──────────────────
  text: string;               // 荧光笔: PDF.js 自动提取; 方框区域: 默认空可手动填
  note: string;
  color: string;
  tags: string[];
...
| **文本内容: text** | 搜索/关系/预览 | 从 PDF.js `getTextContent()` 提取（荧光笔自动，方框区域默认空） | 实时提取，创建时缓存 |
| **用户自定义: previewNote** | 自定义预览笔记 | 可选 MD 文本（默认留白） | 用户自行填入（截图→AI→MD） |
| **可视化: previewNote** | 自定义预览笔记 | 可选 MD 文本（默认留白，用户自行填入） | 截图→AI→MD文本 |
...
interface PDFAnnotationCreateParams extends AnnotationCreateParams {
  ...
  pdfSelector: PDFSelector;
  previewNote?: string;       // 可选 MD 文本预览（v1.1: 替代 previewImage）
}
```
...
5. previewNote 默认留白，用户可后续填入 MD 文本
...
| **PDF highlight** | **提取文本 + previewNote(MD渲染)** | **打开 PDF 跳转页码** |
| **PDF area** | **previewNote(MD渲染, 默认留白)** | **打开 PDF 跳转区域** |
...
### Phase 2: 区域标注 + previewNote (1-2 周)

| 任务 | 工作量 | 优先级 |
|------|--------|--------|
| 矩形拖拽选择工具 | 2天 | P0 |
| 归一化坐标存储 + 缩放还原 | 1天 | P0 |
| previewNote MD 文本字段支持 | 1天 | P1 |
| PDF 标注工具栏 UI | 1天 | P1 |
...
> v1.1 校准: 去掉截图预览(canvas.toDataURL → base64)，改为 previewNote 可选 MD 文本字段。
> 用户可自行截图→会话AI→获取MD文本→粘贴到 previewNote，标注详情页支持 MD 渲染。
...
- [ ] Annotation 接口扩展设计评审（docType + pdfSelector + previewNote）
...
| PDF 标注预览 | 截图 base64 vs 文本提取 vs previewNote(MD) | **previewNote(MD文本)** | v1.1校准: 默认留白，用户自行填入，支持MD渲染 |
